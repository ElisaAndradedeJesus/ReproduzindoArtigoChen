...
// Chave do Fluxo (Agora um 5-tuple completo)
struct flow_key {
    __u32 src_ip;    // 4 bytes
    __u32 dst_ip;    // 4 bytes (NOVO)
...
int network_flow_monitor(struct xdp_md *ctx) {
    ...
    struct flow_key key = {0}; // Zera o padding também
    key.src_ip = ip->saddr;
    key.dst_ip = ip->daddr;    // Captura o IP de Destino
    ...
    if (!metrics) {
        ...
        new_metrics.dst_ip = key.dst_ip;   // Salva o IP de Destino na métrica
